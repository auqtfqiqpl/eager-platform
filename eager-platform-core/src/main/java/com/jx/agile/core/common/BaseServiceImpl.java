...
public class BaseServiceImpl<M extends BaseMapper<T>, T>  implements IBaseService<T> {
    ...
    public boolean update(Wrapper<T> updateWrapper, Integer expectSize) {
        Subject subject = SecurityUtils.getSubject();
        Account currentAccount = (Account) subject.getPrincipal();

        int updateResult = baseMapper.update(null, updateWrapper);

        return updateResult == expectSize;
    }

    @Override
    public boolean remove(Wrapper<T> wrapper, Integer expectSize) {
        int delResult = baseMapper.delete(wrapper);
        return delResult == expectSize;
    ...
    @Override
    public boolean removeByIds(Collection<? extends Serializable> idList,int expectSize) {
        int delSize = baseMapper.deleteBatchIds(idList);
        return SqlHelper.retBool(delSize) && delSize == expectSize;
    }
